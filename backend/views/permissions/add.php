...
<div class="container">
    ...
    <div class="form-all">
        ...
        <form action="<?=Url::to('add')?>" method="post">
            <p>
             <span>
              管理用户组：<input type="text" value="" id="groupname" name="groupname" required="required" placeholder="组名字"/>
    ...
    <table cellspacing="0" class="table2 table-striped">
        ...
        <tr>
            ...
            <td><?=$key?></td>
            <td> <input type="checkbox" name="access[]"  value="<?= $key?>"> <?=$key?></td>
            <td id='ck<?=$i?>'>
                <?php foreach($menu as $k=>$v):?>
                <label class="checkbox-inline">
                    <input type="checkbox" name="name[]" value="<?= $k?>"> <?=$k?> </label>
                <?php endforeach;?>
                <label class="checkbox-inline">
...
<script type="text/javascript">
    $(function () {
        ...
        $('#b_btn').click(function(){
            ...
            $.post("<?=Url::to('add')?>",from,function(data){
                var data=eval('('+data+')');
                if(data.status==1){
                    showLayer(data)
                }else {
