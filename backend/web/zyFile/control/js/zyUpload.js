	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					filename:'UploadForm[fileList]',		//上传表单名字
					/* 提供给外部的接口方法 */
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
                    ...
                    html += '							<img class="upload_image" src="/zyFile/control/images/addimg.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
                    html += '						</div>';
                    html += '					</a>';
                    html += '				</div>';
					html += '			</div>';
					html += '		</div>';
					html += '			<div class="status_bar">';
					html += '				<div id="status_info" class="info">选中0张图片,共0KB。</div>';
					html += '				<div class="btns">';
					html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
					html += '					<div class="webuploader_pick">' +
							//'<img src="control/zyFile/control/images/addimg.png" alt="">' +
						'</div>';
					html += '					<div class="upload_btn">开始上传</div>';
					html += '				</div>';
					html += '			</div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "control/zyFile/control/images/fileType/";
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						para.onSuccess(file, response); //回调外面成功函数
